...
The exact canonical-profile parameters, nonrecursive WorkIntent core, and side-by-side WorkIntent 0.2 / canonical WorkLease 0.2 / WorkContract 0.3 successor cohort are now machine-bound without issuing any canonical identity. Their immutable predecessors remain resolvable. The next architecture mission replaces the synthetic source-view and planning-epoch values and repairs the output-schema digest mismatch through new immutable successors, then issues/adopts the profile before any canonical digest or admission can exist. The verification-assignment cohort, T1 AuthorityAssignment, command/event/state/reducer graph, constitutional root/checkpoint/activation chain, independent reducers/verifiers, replay/recovery/correction-fanout evidence, rights-settled proof import, canonical migration closure, accountable reviews, exact candidate manifest, and Daniel’s exact-byte decision remain mandatory before Gate A. The [closure plan](docs/GATE_A_PREREQUISITE_CLOSURE_PLAN_2026-07-16.md) and [current handoff](docs/SESSION_HANDOFF.md) retain the dependency order and open limitations.
