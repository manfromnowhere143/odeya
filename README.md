...
> **Current state — 2026-08-03:** architecture foundation only. No executable research engine, autonomous-science capability, comparative research-value result, production deployment, or automatic publication is claimed. Gate A remains blocked; runtime stays prohibited until Gate A is accepted, any separately authorized Gate B probes are settled, and Gate C explicitly authorizes one bounded increment.

Current bytes do not support a claim that Odeya is generally useful, frontier,
or superior to any named comparator.
[ADR 0110](docs/decisions/0110-require-matched-budget-frontier-value-evidence-before-superiority-claims.md)
defines the evidence a future candidate would need before such a claim could
be considered.
...
```mermaid
flowchart TB
    accTitle: Odeya architecture evidence, gate path, proposed engine, and governed release
    accDescr: Settled PRQ-002F through PRQ-002I architecture evidence leads to open Gate A dependencies. Gate A remains blocked. The proposed, unimplemented engine separates contract, execution, evidence, verification, adjudication, learning, and human-governed external release.

    subgraph F["ARCHITECTURE EVIDENCE · SETTLED LINEAGE + LOCAL REVIEW SUBJECT"]
        direction LR
        F2["SETTLED PRQ-002F–I PUBLIC MACHINE CHAIN<br/>raw-aware traces · JCS serialization · nine-domain governance<br/>offline dependency closure · baseline 1d5cfe8c"]
        F1["LOCAL G0/G5 REVIEW CANDIDATE<br/>truth reconciliation + frontier-value protocol<br/>publication not authorized"]
        F2 --> F1
    end

    subgraph S["ARCHITECTURE GATES · CURRENT AUTHORITY BOUNDARY"]
        direction LR
        Q["OPEN GATE A DEPENDENCIES · A-001–A-016<br/>selected active path: accountable review · admission-evidence completeness<br/>operator profile decision · first G0/G5 wedge"]
        GA{"GATE A · BLOCKED<br/>architecture not accepted"}
        GB["GATE B · NOT AUTHORIZED<br/>separately scoped disposable probes"]
        GC["GATE C · NOT REACHED<br/>separate bounded implementation decision"]
        I["IMPLEMENTATION SLICE<br/>NOT BUILT"]
        Q --> GA
        GA -.->|after acceptance; separately authorized| GB
        GA -.->|after acceptance + any authorized probes settled| GC
        GB -.->|findings integrated| GC
        GC -.->|one bounded increment| I
    end

    subgraph R["ODEYA · PRIVATE RESEARCH ENGINE · PROPOSED CONTROL ARCHITECTURE · NOT IMPLEMENTED"]
        direction LR
        C["1 · CONTRACT + 2 · COMPILE<br/>ResearchMissionSpec · protocol · falsifiers<br/>rights · resources · authority · immutable run manifest"]
        K[("3 · EXECUTE + 4 · RETAIN EVIDENCE<br/>isolated work · content-addressed artifacts<br/>CANONICAL SCIENTIFIC STATE<br/>append-only event + evidence ledger · deterministic replay")]
        V["5 · VERIFY + 6 · ADJUDICATE + 7 · LEARN<br/>Separately assigned verification role<br/>replication · falsifiers · bounded determination<br/>eligible claim projection · Grounded memory preserves<br/>failure · correction · unknowns"]
        C --> K --> V
    end

    subgraph X["FUTURE SCIENTIFIC / PRODUCT RELEASE PATH · adjudicated candidate only · separate from scientific truth"]
        direction LR
        RC["Release candidate"]
        H{"Human release decision<br/>exact candidate · PRQ-013 blocked"}
        M["Exact manifest<br/>candidate + decision"]
        PG["Exact single-use grant"]
        ...
        RC --> H
        H -->|authorized exact candidate| M --> PG --> E --> O
        H -->|denied or no current valid decision| N
    end
    ...
    class F2 retained
    class F1 candidate
    class Q,GB,GC,I quiet
    class GA blocked
    class C,V core
    class K state
    class RC,M,PG,E,O release
    class H releaseGate
    class N quiet
    style F fill:#F0FDF4,stroke:#047857,stroke-width:2px
    style S fill:#F8FAFC,stroke:#64748B,stroke-width:1.5px
    style R fill:#F8FAFC,stroke:#0F172A,stroke-width:2px
    style X fill:#FFFBEB,stroke:#9A3412,stroke-width:2px
```

The evidence band distinguishes the settled PRQ-002F–I public machine chain
from the blue local G0/G5 review candidate. A tracked file cannot contain the
hash of the commit that contains it, and rehearsal or external settlement can
change after its bytes are fixed; resolve the exact `HEAD`, tree, local
evidence manifest, permanent release ref, public `main`, and remote replay from
Git and subject-bound receipts before acting. The local candidate has no
publication authority and inherits no exact-subject release evidence from its
parent. The gate node shows a selected active path through A-001–A-016, not the
complete blocker ledger; the [current status](docs/ARCHITECTURE_STATUS.md) is
authoritative. The blocked gate prevents architecture bytes from being
mistaken for a built engine. The lower engine is the proposed control
architecture, not a runtime screenshot. The orange path is future
assurance-mediated scientific or product release; architecture-repository
publication is the separate ADR 0047/0091 exact-commit exception.
...
Odeya's requirements are informed by three bounded proof-mission snapshots
rather than invented from an abstract agent demo:
...
| **Inbar** | Evidence-admissibility and authority requirements for future physical causal studies and prospective intervention tests; the current substrate result is blocked |
...
30 intended counterexamples. These counts are bound to the validator run that
...
The later settled PRQ-002F–I sequence closes the current machine units that
ADR 0103 intentionally left downstream, without amending its frozen subjects.
[ADR 0106](docs/decisions/0106-prove-raw-aware-numeric-trace-conformance-for-the-0-3-cohort.md)
retains complete raw-aware traces and cross-object conformance over the exact
fifteen-subject cohort;
[ADR 0107](docs/decisions/0107-prove-profile-bounded-jcs-serialization-conformance.md)
retains profile-bounded JCS serialization and refusal evidence;
[ADR 0108](docs/decisions/0108-prove-nine-product-domain-frame-governance.md)
retains governance/refusal evidence for the nine frozen product-domain
nonidentity frames; and
[ADR 0109](docs/decisions/0109-prove-dependency-closed-offline-resolution.md)
retains two source-separated projections plus third-path validation over one
declared 199-member universe, 6,456 offline schema-reference edges, and 241
declared digest bindings. Four historical identities remain explicitly
unresolved. No product identity, profile issuance/admission, organizational
independence, accountable review, operator acceptance, PRQ-002 closure, Gate A
acceptance, or runtime authority follows.

[ADR 0110](docs/decisions/0110-require-matched-budget-frontier-value-evidence-before-superiority-claims.md)
adds a proposed, unexecuted G0/G5 claim boundary and
[frontier-value evaluation protocol](docs/FRONTIER_RESEARCH_VALUE_EVALUATION_PROTOCOL_2026-08-03.md).
It defines how a future exact engine candidate could be compared against
deterministic, human/current-practice, strong single-agent, and strong
multi-agent/product baselines. It reports no Odeya result and passes no gate.
...
The proposed architecture specifies a modular scientific kernel with isolated
cognitive workers around it:
...
Etzio is a separate research mission and untrusted evidence source—not an Odeya
proof mission, runtime dependency, namespace, control plane, or authority.
...
- [Frontier research value evaluation protocol candidate](docs/FRONTIER_RESEARCH_VALUE_EVALUATION_PROTOCOL_2026-08-03.md)
- [Repository release engineering](docs/REPOSITORY_RELEASE.md)
- [Roadmap](docs/ROADMAP.md)

## Reproduce the checkpoint

The architecture validator runs without a development server. The pinned
checkpoint interpreter is Python 3.14.2:
...
```bash
ODEYA_AUDIT_PYTHON="$(pwd)/.venv-architecture/bin/python"
"$ODEYA_AUDIT_PYTHON" scripts/audit_lifecycle_guard_coverage.py \
  --check --python "$ODEYA_AUDIT_PYTHON"                # ~90s
"$ODEYA_AUDIT_PYTHON" scripts/audit_suite_guard_coverage.py \
  --check --python "$ODEYA_AUDIT_PYTHON"                # duration varies
```
...
remote-main evidence. Corrected replacement sibling `1c6fb11` then completed the
...
Subsequent exact-subject PRQ-002F `741b7b6`, PRQ-002G `3ada167`, PRQ-002H
`1cf0524`, and PRQ-002I `1d5cfe8` candidates each completed their own ordinary
publication sequence. Resolve their full commits and receipts from Git and the
handoff; the settled public PRQ-002I baseline was
`1d5cfe8c232928bbfdb8f9893cb601838e987b4f` at this candidate's 2026-08-03
recovery audit.

## Next

The settled public lane's bounded machine chain is complete through PRQ-002I:
raw-aware traces, profile-bounded serialization, nine-domain governance, and a
declared 199-member universe whose 6,456 schema-reference edges and 241
declared digest bindings resolve from repository bytes. Four historical
residue identities remain named rather than resolved. The remaining PRQ-002
work has one architecture-owned prerequisite—specify and prove a complete,
non-omittable admission-evidence commitment—and two deliberately
non-self-suppliable prerequisites: accountable non-author review and the
operator's exact-byte `odeya-jcs-0.3` decision. Existing root/C0/activation
candidates can digest evidence references, but no retained proof yet binds
every stable member-content digest to the exact conformance, trace, and review
bundle used for admission. No observer agreement, structural vector, or green
validator may be promoted into product identity, profile issuance, PRQ-002
closure, Gate A acceptance, or runtime authority.

The current local architecture candidate repairs stale reviewer-facing status
surfaces and proposes a falsifiable frontier-value evaluation contract. It is
not preregistered or executed, changes no profile/schema byte, reports no
research-value result, and has no repository-publication authority. G0 and G5
remain partial until the operator chooses one rights-cleared first wedge and
accountable domain/statistical review accepts exact tasks, comparators, budgets,
graders, thresholds, known-bads, and consequence rules.
