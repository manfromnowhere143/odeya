"""Deterministically regenerate the PRQ-002H suite metadata files.
...
def generated_files() -> dict[str, bytes]:
    python_lock = {
        ...
        "artifact_class": "prq_002h_product_domain_frame_dependency_lock",
        "suite_id": SUITE_ID,
    ...
    package_manifest = {
        "name": "prq-002h-domain-frames-node-runner",
        "version": "0.1.0",
        "private": True,
        "description": (
            "Zero-dependency source-separated Node.js runner for the PRQ-002H "
            "nine-product-domain frame governance suite. Architecture "
    ...
    def source_manifest(
        ...
        return encode(
            {
                "schema_version": "0.1.0",
                "artifact_class": "prq_002h_product_domain_frame_source_manifest",
                "suite_id": SUITE_ID,
                "role": role,
                ...
                "allowed_input_roles": ["repository_cohort", "vectors", "source_manifest"],
    ...
    files["python/source-manifest.json"] = source_manifest(
        ...
        "Python",
        "3.14.2",
        "stdlib_raw_decode_with_lexeme_hooks_and_closed_evaluator",
        [
            ("runner", "tests/profile-0-3-product-domain-frames/python/runner.py"),
            (
                "dependency_lock",
    ...
    files["node/source-manifest.json"] = source_manifest(
        "node",
        "nodejs-native-domain-governor.0001",
        "JavaScript",
        "24.18.0",
        "native_recursive_descent_with_closed_evaluator",
        [
            ("runner", "tests/profile-0-3-product-domain-frames/node/runner.mjs"),
            (
                "package_manifest",
...
def main() -> int:
    ...
    if arguments.check and failures:
        print(
            "PRQ-002H suite metadata differs from deterministic regeneration: "
            + ", ".join(sorted(failures))
        )
        return 1
    print(
        ("verified" if arguments.check else "generated")
        + f" {len(files)} deterministic PRQ-002H metadata files"
    )
    return 0
