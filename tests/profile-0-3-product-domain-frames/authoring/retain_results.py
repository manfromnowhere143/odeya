"""Retain the PRQ-002H dual-implementation results and receipts.
...
RESULT_PATHS = {
    "python": f"{SUITE_PREFIX}/results/python-frames-result.json",
    "node": f"{SUITE_PREFIX}/results/node-frames-result.json",
}
RECEIPT_PATHS = {
...
PREDECESSOR = {
    "commit": "3ada167aae4ec7f6855de395ba7636dbae78ed57",
    "tree": "b157a829ea0b8ccea85b8c87c956045b12264030",
}
...
def run_runner(executable: Path, runner_relative: str, source_manifest_relative: str, *, node: bool):
    ...
    if document.get("artifact_class") != "prq_002h_product_domain_frame_result":
...
def main() -> int:
    ...
    def receipt(role: str, execution: dict, stdout: bytes, runtime: dict) -> dict:
        return {
            ...
            "artifact_class": "prq_002h_product_domain_frame_execution_receipt",
            "receipt_id": f"prq-002h-{role}-execution.0001",
            "suite_id": SUITE_ID,
            "implementation_id": (
    ...
    comparison = {
        "schema_version": "0.1.0",
        "artifact_class": "prq_002h_product_domain_frame_comparison_receipt",
        "comparison_id": "prq-002h-domain-frames-comparison.0001",
        "suite_id": SUITE_ID,
        "predecessor_checkpoint": PREDECESSOR,
    ...
    with tempfile.TemporaryDirectory(
        prefix=".prq002h-staging-", dir=SUITE
    ) as staging_name:
    ...
    print(
        "retained PRQ-002H observation transaction: "
