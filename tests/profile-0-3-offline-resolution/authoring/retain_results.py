"""Retain the PRQ-002I dual-implementation results and receipts.
...
UNIVERSE = ROOT / SUITE_PREFIX / "universe-manifest.json"
RESULT_PATHS = {
    "python": f"{SUITE_PREFIX}/results/python-resolution-result.json",
    "node": f"{SUITE_PREFIX}/results/node-resolution-result.json",
}
RECEIPT_PATHS = {
...
PREDECESSOR = {
    "commit": "1cf0524744b194515964b708d75ae1b8bd39fb2c",
    "tree": "c9e733b3222f02fc12d9e2d15d9e81ec98491581",
}
...
def run_runner(executable: Path, runner_relative: str, source_manifest_relative: str, *, node: bool):
    ...
    if document.get("artifact_class") != "prq_002i_offline_resolution_result":
...
def main() -> int:
    ...
    def receipt(role: str, execution: dict, stdout: bytes, runtime: dict) -> dict:
        return {
            ...
            "artifact_class": "prq_002i_offline_resolution_execution_receipt",
            "receipt_id": f"prq-002i-{role}-execution.0001",
            "suite_id": SUITE_ID,
            "implementation_id": (
            ...
            "universe_binding": binding(f"{SUITE_PREFIX}/universe-manifest.json"),
            "runtime": runtime,
    ...
    comparison = {
        "schema_version": "0.1.0",
        "artifact_class": "prq_002i_offline_resolution_comparison_receipt",
        "comparison_id": "prq-002i-offline-resolution-comparison.0001",
        "suite_id": SUITE_ID,
        "predecessor_checkpoint": PREDECESSOR,
        "suite_manifest_binding": binding(f"{SUITE_PREFIX}/manifest.json"),
        "universe_binding": binding(f"{SUITE_PREFIX}/universe-manifest.json"),
        "universe_generator_binding": binding(f"{SUITE_PREFIX}/authoring/generate_universe.py"),
    ...
    with tempfile.TemporaryDirectory(
        prefix=".prq002i-staging-", dir=SUITE
    ) as staging_name:
    ...
    print(
        "retained PRQ-002I observation transaction: "
