"""Deterministically regenerate the PRQ-002I suite metadata files.
...
def generated_files() -> dict[str, bytes]:
    python_lock = {
        ...
        "artifact_class": "prq_002i_offline_resolution_dependency_lock",
        "suite_id": SUITE_ID,
    ...
    package_manifest = {
        "name": "prq-002i-offline-resolution-node-runner",
        "version": "0.1.0",
        "private": True,
        "description": (
            "Zero-dependency source-separated Node.js runner for the PRQ-002I "
            "offline-resolution and binding-replay suite. Architecture "
    ...
    def source_manifest(
        ...
        return encode(
            {
                "schema_version": "0.1.0",
                "artifact_class": "prq_002i_offline_resolution_source_manifest",
                "suite_id": SUITE_ID,
                "role": role,
                ...
                "allowed_input_roles": ["repository_universe", "universe_manifest", "source_manifest"],
    ...
    files["python/source-manifest.json"] = source_manifest(
        ...
        "Python",
        "3.14.2",
        "stdlib_strict_pairs_with_offline_registry_walk",
        [
            ("runner", "tests/profile-0-3-offline-resolution/python/runner.py"),
            (
                "dependency_lock",
    ...
    files["node/source-manifest.json"] = source_manifest(
        "node",
        "nodejs-native-offline-resolver.0001",
        "JavaScript",
        "24.18.0",
        "native_recursive_descent_with_offline_registry_walk",
        [
            ("runner", "tests/profile-0-3-offline-resolution/node/runner.mjs"),
            (
                "package_manifest",
...
def main() -> int:
    ...
    if arguments.check and failures:
        print(
            "PRQ-002I suite metadata differs from deterministic regeneration: "
            + ", ".join(sorted(failures))
        )
        return 1
    print(
        ("verified" if arguments.check else "generated")
        + f" {len(files)} deterministic PRQ-002I metadata files"
    )
    return 0
