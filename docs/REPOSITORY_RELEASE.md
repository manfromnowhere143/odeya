...
| `Architecture evidence` | Reproduce twelve dedicated prerequisite/member checks: Gate A prerequisites, PRQ-009 order, schema reissue, module manifest, first-slice scope, the PRQ-002A identity probe, the PRQ-002C raw-number prerequisite, the PRQ-002D prehash registry replay, the PRQ-002E profile-0.3 construction, human-decision-assurance successor recomputation, the PRQ-002F numeric-trace conformance recomputation, and the PRQ-002G JCS serialization conformance recomputation. `Foundation` separately runs the complete integrated seventeen-check census, including canonicalization-evaluator integrity, lifecycle/suite guard and condition audits, schema-rule ablation, refusal attribution, the PRQ-002C raw-number observation, the PRQ-002D replay, the PRQ-002E construction check, the human-decision-assurance successor check, the PRQ-002F numeric-trace conformance check, and the PRQ-002G JCS serialization conformance check | Dedicated evidence log plus the integrated Foundation log and pinned safe/known-bad inventories | Not profile issuance or conformance, product identity, registry admission, activation, or an acceptance decision |
...
The release checker compares the dedicated twelve-command run body and the
integrated seventeen-member Python tuple to exact inventories, pins the complete
dedicated job and integrated-validator bytes, and binds the one executable
consumption loop.
The exact inventory contains thirty-one retained known-bad mutations:
each member is removed once, then an out-of-body thirteenth workflow step and a
