...
- **Guard coverage has explicit denominators, not whole-repository coverage.**
  ...
  `audit_suite_guard_coverage.py` record measures seventeen declared isolated
  contract-checker subjects at 501 of 1260 refusal statements proved, with 759
  explicitly unproved and zero crash-only detections. PRQ-013 contributes
  206/287, leaving 81 explicitly unproved;
  `prq-002-identity-cohort` contributes 0/127, leaving all 127 explicitly
  unproved; `product-identity-profile-candidate` contributes 0/97, leaving all
  97
  explicitly unproved; and the new
  `product-identity-profile-0.3-candidate` contributes 24/72, leaving 48
  explicitly unproved. Under ADR 0104, proof requires a syntax-valid isolated
  mutation, exact suite-reported refusal, passing restored control, and
  identical repeated framed SHA-256 fingerprint. Crash-like outcomes, failed
  controls, and unstable repeats stay unproved; the predecessor-profile suite
  runs serially after the fixed four-worker phase. The unpublished 502/1260
  attempt and former 16-subject 477/1184 measurement are superseded. The
  former 431/820 result is retracted: its assurance-checker mutations tripped
  the unrefreshed outer checker-byte binding rather than the intended guards.
  Central
  ...
  own pinned known-bad self-tests. The generalized audit provides no exact
  case-ID attribution, hostile-writer/process isolation, or independent
  reproduction. The 759 open isolated-suite guards are
...
- **PRQ-013 now has retained byte-bound/recomputation candidate evidence, not
  ...
  `changed_candidate_pending_accountable_closure_review`. The census retains
  the exact 112-schema predecessor partition, the 8 HDA mechanism schemas, 12
  explicitly non-HDA/non-authority PRQ-002B successor schemas, and 12
  explicitly non-HDA/non-authority PRQ-002E construction schemas, for an exact
  144-schema current union: 19 direct or policy-conditional decision rows and
  9 pending operator-acceptance rows.
  Zero current consumers are migrated. No admitted assurance record or
  `AssuredDecision` wrapper identity, T1/T2 dependency chain, real protected
  ceremony, end-to-end consumer refusal, accountable review, operator
  acceptance, or Gate A authority exists.
- **PRQ-002A now has bounded structural-probe evidence, not PRQ-002
  closure.** ADR 0099 adds nine architecture-only schemas outside the 120
  product-schema census, 21 non-issuable probe objects, 20 structured digests,
  and 47 isolated cases: one safe and 46 known-bad. Two source- and
  language-separated retained results carry provenance-attested current
  recomputation. This does not establish independence or issue any probe
  schema, member, snapshot, or profile. No `EngineContractRoot`, activation,
  runtime, PRQ-002 closure, or Gate A authority exists. Product continuation
  requires newly identified product member schemas and domains, a reviewed
  profile successor/reissue, and an explicit migration; no probe identifier
  may be promoted into a product identity.
- **PRQ-002B now has bounded product-identity/profile successor evidence, not
  PRQ-002 closure.** ADR 0100 adds four standalone product-member schemas, one
  ordered-map commitment schema, four pure-registry successor schemas, and
  three schemas plus exact records for a scoped `odeya-jcs-0.2`
  core/evidence/migration chain. The nine product-identity schemas use nine
  distinct domain contracts. Exact current bytes and the frozen 120-schema
  predecessor manifest reconcile to a disjoint 132-schema candidate union;
  the predecessor audit remains frozen at 120 schemas and 216 fixtures. The
  isolated product suite passes one safe control and 19 attributed
  single-fault known-bads, while the Gate cross-check rejects 21 mutations:
  16 exact-inventory or JSON-type attacks and 5 schema-valid semantic
  downgrades. Integral floats cannot satisfy integer byte/count fields, and
...
- **PRQ-002C now has bounded raw-number microframe evidence, not profile
  ...
  61 opaque, answer-free synthetic frames: 9 accepted and 52 refused. Two
  source- and language-separated implementations agree on the complete
  ordered staged projection. The input manifest, Python and Node
  implementations, retained results, and execution receipts are generation
  `.0003`; the corrected comparison receipt is `.0004` and the
  corrected expectation manifest is `.0005`. Fresh recomputation uses two
  distinct challenges, requires the selected executable's pre/post binding to
  remain equal, binds the retained input and source manifests once, and
  reproduces all 61 projections. It does not retain pre/post snapshots of
  those input or source files. The retained execution receipts are self-attested
  byte-consistency records, not independently witnessed historical process
  captures. The stale implementation-causal-binding known-bad refuses only an
  inconsistent relabel; a coherently relabelled copied output and causal
  execution origin remain unproved without an independent witness. All 44
  suite-gate known-bads fire, including exact attacks on
  JSON type/shape, strict parsing, suite inventory, dependency and source
  import boundaries, timestamps, comparison, scope, independence,
  attestation, and authority; they are not semantic-branch source ablations.
  The fixed frames exercise only integer-type and integer-valued-const
  positions. They do not prove generic schema-path evaluation,
  number-position semantics, unique instance-pointer retention, exclusion of
  dynamically discovered paths, nine-domain framing, ordered-map laws,
  cross-object replay, complete offline resolution, organizational
  independence, independent-host reproduction, or full successor-profile
  conformance. The exact `odeya-jcs-0.2` core, evidence, migration, and
  twelve-resource cohort remain byte-for-byte frozen, unissued, and blocked
  from conformance and issuance. The separate PRQ-002E `odeya-jcs-0.3`
  construction does not amend these bytes or supply the missing conformance
  evidence. No product identity, PRQ-002 closure, Gate A
  acceptance, runtime authority, profile issuance authority, or
  scientific-results publication authority follows. Architecture-repository
  publication remains separately governed by the exact-commit release
  contract.
- **PRQ-002D now has bounded non-product prehash replay evidence, not profile
  conformance.** ADR 0102 retains one contract rooted at exact closed
  predecessor commit `d3ec64f3`, its tree, and both cited historical blobs;
  68 opaque virtual-file frames; one fixed private oracle; complete Python and
  Node results; two self-attested execution receipts; and one exact comparison
  receipt. One frame is accepted and 67 are refused under the frozen
  precedence order. The two child implementations are source- and
  language-separated, consume neither the expectation manifest nor the peer
  source/result, and run under exact CPython, Node, npm, and direct-dependency
  bindings. The parent retains 77 known-bad mutations; each must execute the
  production guard and return its declared singleton code. Evidence authoring
  stages and validates the complete generated graph outside the retained
  suite, fsyncs the staged bytes, and replaces the comparison receipt last.
  The receipts remain self-attested byte-consistency records, not
  independently witnessed process evidence; the exact source inventories are
  not process sandboxes, transitive dependency proofs, or general static
  analyses. The bounded replay precedes every structured or product identity
  and does not establish canonicalization-profile conformance, complete
  offline resolution, dependency-closed product registries, cross-object
  product replay, organizational independence, product identity, admission,
  PRQ-002 closure, Gate A acceptance, runtime authority, or publication
  authority. The exact `odeya-jcs-0.2` bytes remain frozen and unissued. The
  later PRQ-002E construction does not enlarge this replay's scope.
- **PRQ-002E now has bounded side-by-side profile construction evidence, not
  conformance or identity.** ADR 0103 retains twelve final-only
  `odeya-jcs-0.3` schema resources, nine schema-valid
  structural-nonidentity fixtures, three ordered candidate records, and a
  separate seven-output observation transaction over the resulting fifteen
  immutable subjects. The product-authoring transaction deterministically
  reproduces exactly 24 outputs and preserves every frozen `0.2` byte. Two
  source- and language-separated observers agree on the complete exact-byte
  fifteen-row projection. Their raw-number literal census is not static
  applicability proof. Complete per-subject raw-aware traces, full
  source-separated profile and cross-object conformance, dependency-closed
  offline resolution, organizational independence, independent-host
...
- **PRQ-002H now has bounded nine-domain frame-governance evidence, not
  product admission.** ADR 0108 retains a 54-frame answer-free corpus: the
  nine frozen structural-nonidentity fixtures accepted as governed instances
  of their nine product-domain schemas — acceptance structurally reserved
  for bytes identical to the retained fixtures — with complete
  dual-implementation raw-number token traces, plus five deterministic
  refusal variants per domain (integral fraction, lexical negative zero,
  out-of-range integer, undeclared member, removed first-required member)
  each refusing with a declared singleton code through BOTH implementations.
  The two projections are byte-identical; the parent validator is a
  PRQ-002D-style binding gate (a named design boundary: the dual semantic
  paths are the runners themselves) that byte-binds the corpus to the
  fixtures and governing schemas recomputed from repository bytes and
  rejects 29 embedded known-bads including symmetric coherent forgeries.
  The fixtures remain structural nonidentity evidence: no member, digest,
  identity, admission, issuance, PRQ-002 closure, Gate A acceptance, or any
  authority follows.
- **PRQ-002I now has bounded dependency-closure evidence, not complete
  historical resolution.** ADR 0109 retains a declared 199-member universe —
  the full 144-schema corpus, the canonicalization candidate records and
  raw-number contract, the nine frozen fixtures, and the PRQ-002F/G/H
  retained artifacts — proven dependency-closed by two source-separated
  implementations and a third-path validator that each re-derive all 6,456
  schema reference edges (in-document fragments by exact pointer, absolute
  URN references against the registry with recorded target digests,
  everything else refusing, including `$dynamicRef`) and replay all 241
  declared digest bindings across three declared shapes from repository
  bytes. The three projections agree exactly; 26 known-bads fire with
  declared codes, including injected HTTP and unknown-URN references and a
  bound-target byte drift. Four historical resource identities remain
  outside the universe by design and are named in the manifest:
  `command-contract-registry:0.1.0`, `command-receipt:0.3.0`,
  `work-contract:0.1.0`, and `command-envelope:0.4.0`. Undeclared binding
  shapes are not claimed. No product identity, issuance, PRQ-002 closure,
  Gate A acceptance, or authority follows.
...
- Exact closed public ancestor:
  `3ada167aae4ec7f6855de395ba7636dbae78ed57`
- Canonical remote: `https://github.com/manfromnowhere143/odeya.git`.
  The PRQ-002H settlement is complete: live `main` and the permanent release
...
- The active tranche is the bounded PRQ-002I dependency-closed
  offline-resolution candidate (ADR 0109): one ordinary single-parent direct
  child of the exact predecessor on the named branch, containing the declared
  universe manifest, its dual-implementation retained results, the dedicated
  third-path validator, and the complete registration cascade.
- Architecture-repository publication authority for the PRQ-002I candidate
...
BASE=1cf0524744b194515964b708d75ae1b8bd39fb2c
BASE_TREE=c9e733b3222f02fc12d9e2d15d9e81ec98491581
...
PRQ-002I validation and rehearsal. A successful local manifest still does not
...
### Superseded authoritative 2026-07-31 PRQ-002H identity
...
- Sole architecture branch:
  `agent/t0-prq-002-profile-0-3-replacement-20260730`
- Superseded immediate PRQ-002G predecessor:
  `3ada167aae4ec7f6855de395ba7636dbae78ed57`
- Exact predecessor tree:
  `b157a829ea0b8ccea85b8c87c956045b12264030`
- Exact closed public ancestor:
  `741b7b6283eb17a3dfac2abed92e84344f20f19d`
- Canonical remote: `https://github.com/manfromnowhere143/odeya.git`.
  The PRQ-002G settlement is complete: live `main` and the permanent release
  ref for the exact predecessor were independently observed equal at
  settlement, its four candidate and four post-main workflows each completed
  ten attempt-1 jobs successfully, and its local and remote-main rehearsal
  manifests compare equal. The complete subject-bound receipts live under
  `/Users/danielwahnich/workspace/odeya-release-evidence/` keyed by the exact
  predecessor hash; revalidate them from bytes rather than from this prose.
- The active tranche is the bounded PRQ-002H nine-product-domain frame
  governance candidate (ADR 0108): one ordinary single-parent direct child
  of the exact predecessor on the named branch, containing the answer-free
  54-frame corpus, its dual-implementation retained results, the dedicated
  parent binding validator, and the complete registration cascade.
- Architecture-repository publication authority for the PRQ-002H candidate
  was granted by the operator during the retaining session and is consumed by
  one candidate-and-promotion sequence; any later candidate requires fresh
  explicit operator authority. Profile issuance, scientific publication,
  runtime, application, infrastructure, deployment, external-effect,
...
cd /Users/danielwahnich/workspace/odeya-prq-002-profile-0-3-replacement-20260730
source scripts/ci/sanitize-git-environment.sh
BASE=3ada167aae4ec7f6855de395ba7636dbae78ed57
BASE_TREE=b157a829ea0b8ccea85b8c87c956045b12264030
HEAD_COMMIT="$(git rev-parse HEAD)"
test "$(git symbolic-ref --short HEAD)" = \
  agent/t0-prq-002-profile-0-3-replacement-20260730
test "$(git remote)" = origin
...
path, and a remote-main rehearsal manifest spells its `source_class` as
`remote-main`. A missing or invalid local manifest keeps the sole mission at
PRQ-002H validation and rehearsal. A successful local manifest still does not
establish remote settlement. The intended continuation
state is one clean ordinary single-parent direct child of the exact immediate
predecessor on the named branch. A dirty next-session state, merge, extra
commit, second dirty architecture lane, or path substitution requires a new
...
## Superseded PRQ-002G repository recovery identity — retained chronology

The block below described the PRQ-002G authoring state before its
publication settled. It is retained as chronology only. Do not execute its
restart program or use it to select the current writer.

### Superseded authoritative 2026-07-31 PRQ-002G identity
...
- Sole architecture branch:
  `agent/t0-prq-002-profile-0-3-replacement-20260730`
- Superseded immediate PRQ-002F predecessor:
  `741b7b6283eb17a3dfac2abed92e84344f20f19d`
- Exact predecessor tree:
...
dedicated PRQ-002H validator, the full foundation validator, and the
repository-release validator must all pass from current bytes; and the
external evidence root must contain a successful local exact-subject
fresh-clone rehearsal manifest for that `HEAD`, with settlement receipts if
publication completed. If any condition fails, the sole mission remains
PRQ-002H validation, correction, rehearsal, or settlement.

**Sole successor tranche.** Preserve every frozen predecessor byte and the
complete PRQ-002F, PRQ-002G, and PRQ-002H suites. The next falsifiable
units, in dependency order from ADRs 0103 and 0106–0108:

1. dependency-closed offline resolution and cross-object product replay for
   the complete cohort: prove that every `$ref`, digest edge, and registry
   binding in the retained candidate graph resolves inside the repository
   with no network, file-search, environment, alias, or fallback path, and
   that a cross-object replay over the exact retained subjects reproduces
   every binding from raw bytes alone; and
2. the accountable-review and operator-acceptance prerequisites that no
...
The block below describes the dirty PRQ-002D authoring state before commit
`617209ba480b854a00c6a15cd99ac1d5a18e90ad`. It is retained as chronology
only. Do not execute its restart program, mutate its worktree, or use its
branch/path inventory to select the current writer.
...
- Canonical workspace:
  ...
  edit, stage, normalize, or discard its dirty paths.
- Sole architecture writer:
  `/Users/danielwahnich/workspace/odeya-prq-002-schema-registry-prehash-replay-20260729`
- Sole architecture branch:
  `agent/t0-prq-002-schema-registry-prehash-replay-20260729`
- Exact closed predecessor:
  `d3ec64f3abfc64467c0bc3bfae330d86e2af89b2`
- Exact predecessor tree:
  `69304534a61a7c5d085d183d847285a181eaabfc`
- Canonical remote: `https://github.com/manfromnowhere143/odeya.git`.
  The locally retained `origin/main` resolved to the exact closed predecessor
  at recovery. A local remote-tracking ref is not a live remote observation.
- Recovery authority: the external 2026-07-29 master recovery handoff, closure
  receipt for `d3ec64f3`, and closure SHA-256 inventory supplied by the
  operator. Revalidate their claims from repository and Git bytes; do not
  treat the external prose as Gate A authority.
- Current publication authority: no publication, permanent release-ref
  creation, or push is implied by this tranche. Local validation and
  fresh-clone rehearsal are evidence about one exact commit only.
- Runtime, deployment, scientific-publication, external-effect, and Gate A
  authority: not granted.

Only the sole writer named above may be mutated for PRQ-002D. The canonical
checkout and the retired 2026-07-28 successor-conformance-correction worktree
are evidence sources only. The writer must be either the exact predecessor
with the exact 20 tracked and 27 untracked PRQ-002D paths below, or one clean
ordinary single-parent direct child after commit. No merge, extra commit,
second dirty Odeya lane, or path substitution is an accepted recovery state.

Run first:

```bash
bash -euo pipefail <<'BASH'
cd /Users/danielwahnich/workspace/odeya-prq-002-schema-registry-prehash-replay-20260729
source scripts/ci/sanitize-git-environment.sh
BASE=d3ec64f3abfc64467c0bc3bfae330d86e2af89b2
BASE_TREE=69304534a61a7c5d085d183d847285a181eaabfc
HEAD_COMMIT="$(git rev-parse HEAD)"
...
EXPECTED_TRACKED="$(
  printf '%s\n' \
    ...
    docs/REGISTRY_GRAPH_CONTRACTS.md \
...
EXPECTED_UNTRACKED="$(
  printf '%s\n' \
    ...
    tests/schema-registry-prehash-replay/README.md \
    tests/schema-registry-prehash-replay/authoring/generate_vectors.py \
    tests/schema-registry-prehash-replay/cases.json \
    tests/schema-registry-prehash-replay/fixtures/probes/probe-001.valid.json \
    tests/schema-registry-prehash-replay/fixtures/probes/probe-002.valid.json \
    tests/schema-registry-prehash-replay/fixtures/resources/resource-001.schema.json \
    tests/schema-registry-prehash-replay/fixtures/resources/resource-002.schema.json \
    tests/schema-registry-prehash-replay/fixtures/safe-bundle.json \
    tests/schema-registry-prehash-replay/input-manifest.json \
    tests/schema-registry-prehash-replay/manifest.json \
    ...
    tests/schema-registry-prehash-replay/node/source-manifest.json \
    tests/schema-registry-prehash-replay/python/dependency-lock.json \
    tests/schema-registry-prehash-replay/python/runner.py \
    tests/schema-registry-prehash-replay/python/source-manifest.json \
    tests/schema-registry-prehash-replay/results/comparison-receipt.json \
    tests/schema-registry-prehash-replay/results/node-ajv2020.json \
    ...
    tests/schema-registry-prehash-replay/results/python-jsonschema.json \
    tests/schema-registry-prehash-replay/vectors.json
)"
OBSERVED_TRACKED="$(
  {
    git diff --name-only
...
test "$(git rev-parse "$BASE^{tree}")" = "$BASE_TREE"
test "$REMOTE_MAIN" = "$BASE" || test "$REMOTE_MAIN" = "$HEAD_COMMIT"
if test -n "$OBSERVED_TRACKED$OBSERVED_UNTRACKED"; then
  test "$HEAD_COMMIT" = "$BASE"
  test "$OBSERVED_TRACKED" = "$EXPECTED_TRACKED"
  test "$OBSERVED_UNTRACKED" = "$EXPECTED_UNTRACKED"
else
  test "$(git rev-parse "$HEAD_COMMIT^")" = "$BASE"
  test "$(git rev-list --parents -n 1 "$HEAD_COMMIT" | awk '{print NF}')" = 2
  test "$(git rev-list --count "$BASE..$HEAD_COMMIT")" = 1
...
The exact tracked/untracked split is a pre-commit recovery assertion, not a
license to keep generated evidence stale. Before committing, refresh retained
PRQ-002D evidence from the final validator bytes, run both retained and direct
recomputation modes, run the full foundation and release validators, obtain
read-only adversarial review, and prove the protected canonical paths are
absent from the candidate diff. After commit, rehearse only the exact new
commit from a fresh clone. Resolve every later release or remote claim from
subject-bound receipts.

## Historical repository recovery identity — superseded 2026-07-29

The following 2026-07-28 block is retained only as recovery provenance for the
PRQ-002C correction. Its worktree, branch, baseline, dirty-path census, restart
program, and sole-writer declaration are retired and must not be executed or
used to select the current writer.
...
- Active architecture worktree:
  `/Users/danielwahnich/workspace/odeya-prq-002-successor-conformance-review-correction-20260728`
- Active architecture branch:
  `agent/t0-prq-002-successor-conformance-review-correction-20260728`
- Exact published baseline observed at recovery:
  `a79d86b0a5e9581b3bacb57214cf180df3443566`
- Exact published-baseline tree:
  `d44e9eb4751b97871aa9c995664782a5d031fb48`
- Canonical remote: `https://github.com/manfromnowhere143/odeya` (public;
  created 2026-07-17 under ADR 0047; default branch `main`)
- Measured remote state on 2026-07-28: `origin/main` and permanent
  `release/a79d86b0a5e9581b3bacb57214cf180df3443566` both resolved to the
  exact published baseline. Sixteen permanent release refs exist — `a25d026`,
  `f1f25fd`, `08dbad6`, `8ed5d42`, `f4067b5`, `e0187bb`, `34cad10`,
  `86c0f1e`, `36d798e`, `05c2c3b`, `ce962dd`, `f013b2d`, `b02b378`, and
  `e467bdb`, `5332239`, and `a79d86b` — and none is ever rewritten, deleted, or
  recreated, including for a candidate that fails. The exact `a79d86b`
  local/remote
  rehearsal manifests
  have SHA-256
  `a82d41518454d0ec184f86f175a8a74d6e3080cde470694416103b514281fb1e`
  and `d01cfd0700aaf3f7574a3f5f57f1d18dea4f332a255ec535b58fc02ae203fae`;
  its comparison receipt has SHA-256
  `8fbf59435cca857e731f1f9bf6d40f28c589c8d48375d5e867c51374b987611c`.
  Candidate governance/check receipts have SHA-256
  `9a1f679708266b9d30eb2af209c8f58f24fdbe4d955b6dc030a982d4dd283e47`
  and `c6df5025e6c9d522c402b30b60929b2b1639229e28f6f9cf85342eb88d395ec1`;
  promotion governance and post-main checks have SHA-256
  `c3b2ddfaeb150a143d3e79e6c6b8e34440fef140ec760d6580af2ca48324702b`
  and `fe36ff4de88658ff59bb3e02ec976a27d9954ae86fee7fe4de60b44b182c512a`.
  ...
  2026-07-28. Resolve every receipt's current byte validity through the
  admitted verifiers; do not infer it from this prose.
...
```bash
bash -euo pipefail <<'BASH'
cd /Users/danielwahnich/workspace/odeya-prq-002-successor-conformance-review-correction-20260728
source scripts/ci/sanitize-git-environment.sh
git status --short --branch
git rev-parse HEAD
...
PUBLISHED_TREE=d44e9eb4751b97871aa9c995664782a5d031fb48
HEAD_COMMIT="$(git rev-parse HEAD)"
REMOTE_MAIN="$(git rev-parse origin/main)"
BASE_RELEASE="$(
  git ls-remote --refs origin \
    "refs/heads/release/$PUBLISHED_BASELINE" |
    awk '{print $1}'
)"
HEAD_RELEASE="$(
  git ls-remote --refs origin \
...
EXPECTED_ACTIVE_UNTRACKED="$(
  printf '%s\n' \
    architecture/canonicalization-raw-number-token-contract-v1-candidate.json \
    architecture/canonicalization-raw-number-token-contract.schema.json \
    ...
    tests/product-identity-raw-number-typing/README.md \
    tests/product-identity-raw-number-typing/cases.json \
    tests/product-identity-raw-number-typing/input-manifest.json \
    tests/product-identity-raw-number-typing/manifest.json \
    tests/product-identity-raw-number-typing/node/package-lock.json \
    tests/product-identity-raw-number-typing/node/package.json \
    tests/product-identity-raw-number-typing/node/runner.mjs \
    tests/product-identity-raw-number-typing/node/source-manifest.json \
    tests/product-identity-raw-number-typing/python/dependency-lock.json \
    tests/product-identity-raw-number-typing/python/runner.py \
    tests/product-identity-raw-number-typing/python/source-manifest.json \
    tests/product-identity-raw-number-typing/results/comparison-receipt.json \
    tests/product-identity-raw-number-typing/results/node-execution-receipt.json \
    tests/product-identity-raw-number-typing/results/node-recursive-descent.json \
    tests/product-identity-raw-number-typing/results/python-execution-receipt.json \
    tests/product-identity-raw-number-typing/results/python-stdlib.json \
    tests/product-identity-raw-number-typing/vectors.json
)"
OBSERVED_ACTIVE_TRACKED="$(
  {
    git diff --name-only
    git diff --cached --name-only
  } | LC_ALL=C sort -u
)"
OBSERVED_ACTIVE_UNTRACKED="$(
  git ls-files --others --exclude-standard | LC_ALL=C sort -u
)"
if test -n "$OBSERVED_ACTIVE_TRACKED$OBSERVED_ACTIVE_UNTRACKED"; then
  test "$HEAD_COMMIT" = "$PUBLISHED_BASELINE"
  test "$OBSERVED_ACTIVE_TRACKED" = "$EXPECTED_ACTIVE_TRACKED"
  test "$OBSERVED_ACTIVE_UNTRACKED" = "$EXPECTED_ACTIVE_UNTRACKED"
fi
test "$(git symbolic-ref --short HEAD)" = \
  agent/t0-prq-002-successor-conformance-review-correction-20260728
test "$(git remote)" = origin
test "$(git remote get-url origin)" = \
  https://github.com/manfromnowhere143/odeya.git
test "$(git rev-parse "$PUBLISHED_BASELINE^{tree}")" = "$PUBLISHED_TREE"
test "$REMOTE_MAIN" = "$PUBLISHED_BASELINE" || test "$REMOTE_MAIN" = "$HEAD_COMMIT"
test "$BASE_RELEASE" = "$PUBLISHED_BASELINE"
if test "$HEAD_COMMIT" = "$PUBLISHED_BASELINE"; then
  test "$REMOTE_MAIN" = "$PUBLISHED_BASELINE"
  test "$HEAD_RELEASE" = "$PUBLISHED_BASELINE"
else
  test "$(git rev-parse "$HEAD_COMMIT^")" = "$PUBLISHED_BASELINE"
  test "$(git rev-list --parents -n 1 "$HEAD_COMMIT" | awk '{print NF}')" = 2
  test "$(git rev-list --count "$PUBLISHED_BASELINE..$HEAD_COMMIT")" = 1
  test -z "$HEAD_RELEASE" || test "$HEAD_RELEASE" = "$HEAD_COMMIT"
  if test "$REMOTE_MAIN" = "$HEAD_COMMIT"; then
    test "$HEAD_RELEASE" = "$HEAD_COMMIT"
  fi
fi
...
The dirty pre-commit state is therefore an exact 40-path candidate: 19 tracked
paths and 21 untracked paths. The recovery program fails if either partition
gains, loses, or exchanges one path. A clean direct-child `HEAD` is the only
post-commit alternative; the program does not permit a dirty descendant.
...
- the exact published baseline and tree remain at their permanent
  `release/<sha>` ref, while public `origin/main` is either that baseline before
  a new publication or the exact active `HEAD` after same-SHA promotion;
- the active `HEAD` is either that already-published baseline or one ordinary
  single-parent direct child of it; an unexpected rewind, replacement, merge,
  or multi-commit range fails recovery;
...
- no worktree path is dirty beyond the exact protected and quarantined
  inventories below; and
...
`docs/decisions/README.md` is protected, so ADRs 0092–0105 are intentionally
...
### Quarantined canonical-worktree collision

Recovery on 2026-07-28 found additional untracked PRQ-shaped bytes in the
canonical worktree. Its `agent/repository-release` HEAD is
`4486a721eddcfa93ea87df8f8d0f411b42632838`, an ancestor 106 commits behind
the published `d3ec64f3` closed predecessor. The following paths are
quarantined there:

- untracked: `schemas/aggregate-state-subject-record.schema.json`
- untracked: `schemas/event-contract-record.schema.json`
- untracked: `schemas/reducer-contract-record.schema.json`
- untracked: `schemas/schema-resource-record.schema.json`
- untracked:
  `tests/product-identity-raw-number-typing/results/comparison-receipt.json`
- untracked:
  `tests/product-identity-raw-number-typing/results/node-execution-receipt.json`
...
All nine are outside the declared PRQ-002E construction scope and differ
byte-for-byte from the published predecessor; five overlap historical
PRQ-002C path names. They are not predecessor evidence, candidate inputs, or
an alternate source of truth. Only the PRQ-002E worktree and branch named in
the authoritative current recovery identity above is the architecture writer.
Do not copy, merge, stage, edit, normalize, or delete the quarantined
canonical bytes. Before commit or publication, re-run both worktree
inventories; any new path, byte overlap, or ownership change freezes mutation
again.
...
- unissued `HumanDecisionAssuranceCore`, `HumanDecisionAssuranceEvidence`, and
  `HumanDecisionAssuranceSeal` 0.1.0 schema resource identities, all immutable
  and still unissued;
- an adopted, acyclic two-phase v2 challenge-frame profile and separately
  retained v1/v2 deterministic recomputation vectors; the presentation
  challenge precedes the confirmation receipt and the authentication challenge
  commits to that receipt digest without creating a profile/Core digest cycle;
...
- an isolated checker with 11 safe controls, 232 one-mutation intent-bound
  known-bads, and 161 distinct intent refusal rules.

[ADR 0095](decisions/0095-reissue-human-decision-assurance-as-a-byte-bound-independently-recomputed-chain.md)
adds the retained T0 successor tranche without mutating those predecessor
identities:

- five unissued successor schemas and seven schema-valid successor fixtures;
- exact ruleset 0.2 and its resolver profile;
...
- source-separated, non-sharing Python 3.14.2, Node.js 24.18.0, and Temurin
  ...
  recomputations; and
- exact comparison of `participant_id`, `domain_results`,
  `categorical_results`, `categorical_failures`, `final_disposition`, and
  `reason_codes` before Seal 0.2.
...
The measured candidate inventory is 144 schemas and 908 shared-manifest cases
(244 valid and 664 known-bad), with 17 isolated suites and 15
architecture-evidence checks. The frozen predecessor canonical audit covers
120 schemas and 216 fixtures; all 158 predecessor retained
cross-field rule cases isolate their declared rule. The generalized guard
record is 501/1260 proved across 17 checker subjects, 759 explicitly unproved,
and zero crash-only detections. The preceding 16-subject 477/1184 measurement
and its 0/93 `product-identity-profile-candidate` row are superseded by current
checker bytes; the current rows are 0/97 for that subject and 24/72 for the new
`product-identity-profile-0.3-candidate` subject. Its former 431/820 result is
retracted because an unrefreshed self-binding produced false-positive
detections. The v0.2 record additionally requires syntax-valid, stable
suite-reported refusal with a passing restored control and retained repeated
fingerprint; it does not establish exact case-ID causality or independent
reproduction. The consumer census
partitions all 112 predecessor schemas without changing one byte, classifies
the 8 HDA mechanism schemas, and records all 12 PRQ-002B plus all 12 PRQ-002E
schemas as explicitly non-HDA and non-authority, for an exact 144-schema
union. It
inventories 121 command and 135 event
...
It also compares all frozen baseline-schema and Authority Matrix bytes with
the frozen Git source, requires the exact 112+8+12+12 schema union, resolves every
...
admission. The 0.1 resources and all successors are unissued; no admitted
assurance-record or `AssuredDecision` wrapper identity exists; migrated
consumer count is zero. T1/T2, wrapper construction, end-to-end consumer
refusal, real protected ceremony, accountable security and authority review,
profile issuance, and Daniel's exact-byte acceptance are all absent. The
receipt/authenticator co-binding and backing preimages exist only as synthetic
construction evidence and must not be represented as measured ceremony or
real-person evidence. PRQ-013, T1, Gate A, runtime, cloud access, deployment,
scientific-results publication, spending, data access, and external effects
remain blocked.
...
## 2026-07-21 truth-surface reconciliation

The adopted ADR 0093 bytes exposed a current-state defect in their published
parent: the machine guard record and the later adoption log correctly retained
`469/958` proved guards, `489` unproved guards, and `199/284` for the assurance
checker, while the README, architecture status, active handoff sections, and
guard-closure plan still presented the predecessor `458/927`, `469`, and
`188/253` values. Several current assurance surfaces also still described the
v1 receipt/authenticator limitation after the v2 two-phase construction had
been adopted. The structural validator passed because it checked the machine
record and a different README count sentence, not these public interpretations.

The reconciliation corrects the current README, architecture status, active
handoff, guard-closure plan, threat model, assurance contract and suite
contract, and marks ADR 0092's v1 wording as retained history superseded in
that one detail by ADR 0093. The Gate A prerequisite record now names ADR 0093
and states the exact boundary: cryptographic co-binding is true only as a
synthetic construction property; no real ceremony measured that one natural
person performed both acts. Candidate evidence rebinds every changed artifact
it already claimed to bind.

This is not only an editorial repair.
[ADR 0094](decisions/0094-bind-current-status-surfaces-to-retained-machine-evidence.md)
requires the suite-guard validator to parse the named current sections and the
complete closure-plan table from retained machine evidence; hidden comments,
stale-plus-correct duplicates, relocated text, missing rows, and changed rows
must refuse. The Gate A prerequisite validator applies the same scoped rule to
the current ADR-0093 boundary and recovery identity. Their focused known-bads
are intent-bound rather than credited by an unrelated error from the same
file.
Those passes establish bounded consistency only. Gate A, profile issuance,
accountable review, real ceremony, runtime, deployment, scientific
publication, spending, and external effects remain blocked. Commit, rehearsal,
publication, and remote-settlement status for the containing subject must be
resolved from Git and its external exact-SHA evidence; no prose line may supply
that authority.

One assurance case identifier still retains the v1 phrase
`without-a-cycle-free-receipt-path` even though its mutation now tests the
broader rule that an unissued profile cannot be relabelled issued. This tranche
does not rename it: the retained generalized coverage record binds the checker
bytes but not the case-corpus bytes, so carrying 199/284 across even a
semantic-neutral corpus edit would be unproved. Rename it only with a
...
### 2026-07-28 freeze recovery and PRQ-002C evidence correction

The interrupted writer was
`/Users/danielwahnich/workspace/odeya-prq-002-successor-conformance-20260728`
on `agent/t0-prq-002-successor-conformance-20260728`. Its unpublished clean
commit `dcc9a22ca64c6f21abf1eb17a95083e9a37ef13a` was one direct child of
published `a79d86b0a5e9581b3bacb57214cf180df3443566`. The pre-amend commit
`a97471016b58de8484bd68a0c93c92180a577f1e` had complete local rehearsal
evidence, but its tree differs in `README.md` and its evidence cannot transfer.
The first exact-`dcc9a22` rehearsal failed at `formal-models` after local disk
exhaustion. A second run reached the 16-suite/1,184-guard mutation audit before
the host restart and never emitted a terminal result. Recovery therefore
resumed no internal stage and inferred no pass.

The complete pre-recovery Git and dirty-lane state is preserved outside the
repository at
`/Users/danielwahnich/workspace/odeya-recovery-20260728-freeze/`, created
2026-07-28T22:21:48+03:00. The original branch remains unchanged. Its
disk-failure receipt and the later
operator-stopped receipt remain in separately named subject-bound directories
under `/Users/danielwahnich/workspace/odeya-release-evidence/`; neither is
release authority.

Read-only review then reproduced one evidence-language defect: copying the
Python result, relabelling it as Node, and recomputing the public causal-binding
formula yields the retained Node result exactly. The old known-bad refused only
the stale binding left by an inconsistent relabel; it did not prove coherent
peer-output substitution refusal or causal execution origin. The corrected
direct-child lane named in the current recovery identity therefore:

- renames that known-bad and guard to state the measured consistency property;
- reissues private expectations as `.0005` and the bound comparison as
  `.0004`;
- records the coherent-relabel and historical-witness limitations in
  `README.md`, ADR 0101, the suite README, and this handoff; and
- re-pins the existing exact README, handoff, and recovery truth surfaces.

The recovery correction changes no raw-number vector, parser, retained result,
execution receipt, raw-token contract, frozen `odeya-jcs-0.2` byte, schema
inventory, Gate A authority bit, runtime surface, or deployment surface. The
enclosing corrected commit cannot name itself recursively; resolve it from Git
and require one sole parent equal to the published baseline. No GitHub write,
runtime action, deployment, commercial action, or research publication was
authorized merely by this correction.

Recovery state is `SAFE TO RESUME` until the exact corrected commit has a
complete local rehearsal. It becomes `CLOSED` only after the permanent
`release/<sha>` ref, four candidate workflows and ten jobs, same-SHA `main`,
four new post-main workflows and ten jobs, credential-free remote-main
rehearsal/comparison, and final exact-ref/governance read-back all settle for
that same SHA. The exact next action is the first unmet step in
[Reproduce the current exact `HEAD`](#reproduce-the-current-exact-head) and
the ADR 0091 publication sequence; no predecessor receipt is reusable.
...
the v2 adoption route. At that session's terminal observation, public `main`
was `f4067b5`; this dated record does not assert the current live ref.
...
1. Resolve the current `HEAD` from Git. The exact immediate published predecessor
   ...
   `67c38b895276bf2c804fe192339ce90a8c75ea97`; its sole parent
   `d3ec64f3abfc64467c0bc3bfae330d86e2af89b2` is the closed public
   ancestor. The failed immutable `f1bb98d…` candidate is one retained sibling;
   the intended PRQ-002E handoff is a different clean ordinary single-parent
   direct child of `617209ba`. Reproduce that exact replacement from a fresh
   clone and require the complete ADR-0091 candidate, promotion, post-main,
   remote-replay, comparison, and final-ref settlement. Do not infer
   publication from a local pass, reuse either predecessor's receipts, or let
   a descendant inherit candidate-specific evidence.
2. Preserve the PRQ-002E construction boundary exactly: every frozen
   `odeya-jcs-0.2` byte; twelve final-only side-by-side `odeya-jcs-0.3`
   schemas; nine structural-nonidentity fixtures; three ordered candidate
   records; and the separate seven-output observation transaction over the
   fifteen immutable schema-and-record subjects. The next smallest PRQ-002
   unit is complete raw-aware per-subject applicability traces and full
   source-separated cross-object conformance on those exact bytes, followed by
   complete offline resolution, dependency-closed product members and
   registries, accountable review, and the operator's exact decision. Never
   promote an observer count, probe, or structural vector into applicability
   proof, conformance, or product identity. Do not issue a profile, bind an
   `EngineContractRoot`, construct an activation, imply runtime, or mark
   PRQ-002 resolved.
...
The historical closed public ancestor and predecessor boundary for PRQ-002D is
`d3ec64f3abfc64467c0bc3bfae330d86e2af89b2`, with tree
`69304534a61a7c5d085d183d847285a181eaabfc` and sole parent `a79d86b…`.
Its permanent `release/<sha>` ref and public `main` resolved to that exact
commit at its closure. Its local rehearsal manifest has SHA-256
`18fb9e3fdd836f3c67004977ee2a21ef362adba1070aaf51bcbbe88d9d73d540`;
its remote-main manifest has SHA-256
`e7b46cb08939d4f9047409cb1acb108453dfeddcc1a7b8812b76cac2f98a7c8d`;
and their comparison receipt has SHA-256
`503be59757c98edc3aedca4c2eedbbe99384551300715f0e54a801b6d7f5eb10`.
The candidate and post-main censuses each retained four successful workflows
and ten successful jobs. On 2026-07-30, exact descendant
`617209ba480b854a00c6a15cd99ac1d5a18e90ad` was independently observed at
public `main` and its permanent release ref; its exact subject-bound evidence
is mapped in the authoritative current recovery section above. Revalidate all
external receipts and reobserve live refs before relying on them; this tracked
prose is not a live GitHub observation.
Historical first-activation baseline `f1f25fd…` introduced the ADR 0091 hook,
verifier, fourth workflow, and two-ref helper; both no-bypass rulesets remain
observed. The hook and helper remain local workstation controls
