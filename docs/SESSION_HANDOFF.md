...
PRQ-002H nine-product-domain frame governance tranche (ADR 0108) as one
ordinary single-parent direct child of the exact PRQ-002G predecessor: the
nine frozen structural-nonidentity fixtures traced as governed instances of
their nine product-domain schemas through two source-separated
implementations, with each domain's refusal surface exercised as data. It
amends no frozen byte, issues nothing, computes no digest, and constructs no
product identity.
...
## Current repository recovery identity — authoritative 2026-07-31, PRQ-002H
...
- Exact immediate PRQ-002G predecessor:
