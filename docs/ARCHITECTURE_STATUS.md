...
| --- | --- | --- |
| Foundation corpus | 109 Draft 2020-12 schemas; 654 valid/adversarial shared-manifest cases; 7 founding cross-field groups; 11 mandatory isolated contract suites; 2 bounded architecture-evidence checks; 1 repository release-surface check | The full architecture validator passes. Structural and bounded semantic evidence is not runtime or Gate A evidence |
...
| Registry/root graph | Six registry/root/activation schema subjects validate; the proposed logical manifest v2 covers 9 layers, 30 modules, 47 aggregate owners, 109 schema owners, 121 command owners, and 135 event owners | Ownership and acyclic subject shape are present; no real registry snapshot, root, checkpoint, activation, or handler-set equality exists |
...
| Canonical identity | 64 vectors and 4 metamorphic relations agree across pinned Python and Node implementations; the profile binding passes 1 safe/26 known-bads; the WorkIntent core binding passes 1/28; the side-by-side WorkIntent 0.2 / WorkLease 0.2 / WorkContract 0.3 successor cohort passes 1/37; the current audit covers 109 schemas and 200 fixtures | Candidate parser, RFC 8785, envelope, digest framing, future reference shape, 21 domains, and 8 version axes are frozen for review. Immutable predecessors and exact successor schema/resource edges are now present, but all identities remain unissued. Migration remains blocked at 122 unprofiled time paths, 61 number/decimal findings, 668 unscoped digest fields, 56 divergent common-definition names, 11 unpinned profile bindings, and 236 nonconformant fixture timestamps; all eight new profile/work-identity schemas add zero blockers in those classes |
...
The current 109-resource schema set intentionally does not pretend to be a
...
claiming canonical identity or admission. Four historical resource identities
...
| A-005 | Critical | C7/C8 are bounded, but C5/PRQ-009 remains unresolved: no constructible resolved WorkIntent or complete assignment binding exists; exact local execution/reducer members, materialization/refinement replay, and a real witnessed P0 activation instance are also absent | Reissued resolved WorkIntent/WorkContract/ResearchEvent/EventContractRecord identities; exact 13-event assignment, lease/materialization/sandbox/attempt records; sole Verification reducer record; clear root/C0/checkpoint/witness/P0 chain; composite recovery traces; and independent review |
...
| Work identity predecessor/successor cohort | Retains WorkIntent 0.1, canonical WorkLease 0.1, and blocked WorkContract 0.2 while adding exact side-by-side 0.2/0.2/0.3 successor wrappers and lineage evidence | Successor schemas and dependency edges are present, but synthetic source/planning references, an output-schema digest mismatch, profile issuance, canonical digests, admission, and the assignment cohort remain blocked. None is assignable or executable |
