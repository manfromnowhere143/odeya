...
| --- | --- | --- |
| Foundation corpus | 105 Draft 2020-12 schemas; 631 valid/adversarial shared-manifest cases; 7 founding cross-field groups; 10 mandatory isolated contract suites; 2 bounded architecture-evidence checks; 1 repository release-surface check | The full architecture validator passes. Structural and bounded semantic evidence is not runtime or Gate A evidence |
...
| Registry/root graph | Six registry/root/activation schema subjects validate; the proposed logical manifest v2 covers 9 layers, 30 modules, 47 aggregate owners, 105 schema owners, 121 command owners, and 135 event owners | Ownership and acyclic subject shape are present; no real registry snapshot, root, checkpoint, activation, or handler-set equality exists |
...
| Canonical identity | 64 vectors and 4 metamorphic relations agree across pinned Python and Node implementations; the profile core/evidence binding passes 1 safe reference and 26 known-bads; the WorkIntent core/binding passes 1 safe reference and 28 known-bads; the current audit covers 105 schemas and 200 fixtures | Candidate parser, RFC 8785, envelope, digest framing, future reference shape, 21 domains, and 8 version axes are frozen for review, while the WorkIntent core proves a finite external identity construction. Both identities remain unissued. Migration remains blocked at 122 unprofiled time paths, 61 number/decimal findings, 668 unscoped digest fields, 56 divergent common-definition names, 11 unpinned profile bindings, and 236 nonconformant fixture timestamps; the four new profile/WorkIntent schemas add zero blockers in those classes |
...
The current 105-resource schema set intentionally does not pretend to be a
...
prospective nonrecursive construction. Four historical resource identities
...
| WorkIntent core/candidate/blocked WorkContract 0.1.0/0.1.0/0.2.0 | Separates the identity-free non-authoritative intent subject, its external unissued binding, and a future exact post-assignment contract | WorkIntent 0.1 remains current and unresolved; the exact migration unit is prospective WorkIntent 0.2, WorkLease 0.2, and WorkContract 0.3. None is assignable or constructible; C5/PRQ-009 and the exact 13-event cohort remain blocked |
