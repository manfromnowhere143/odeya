...
| --- | --- | --- |
| Foundation corpus | 112 Draft 2020-12 schemas; 814 valid/adversarial shared-manifest cases; 7 founding cross-field groups; 12 mandatory isolated contract suites; 10 bounded architecture-evidence checks; 1 repository release-surface check | The full architecture validator passes. Structural and bounded semantic evidence is not runtime or Gate A evidence |
| Guard evidence | Lifecycle has dedicated statement coverage at 178/185 and top-level condition coverage at 100/103. The generalized audit separately proves 247/635 refusal statements across 12 declared isolated checker subjects, with 388 explicitly unproved and zero crash-only detections | Statement reachability and top-level condition removal only. The generalized denominator excludes central architecture/release gates such as PRQ-009, which carry their own pinned known-bads. Coverage does not prove a guard enforces the right rule |
...
| Canonical identity | 64 vectors and 4 metamorphic relations agree across pinned Python and Node implementations; the profile binding passes 1 safe/26 known-bads; the WorkIntent core binding passes 1/28; the side-by-side WorkIntent 0.2 / WorkLease 0.2 / WorkContract 0.3 successor cohort passes 1/37; the exact-reference WorkIntent 0.3 cohort passes 1/40; the current audit covers 112 schemas and 200 fixtures | Candidate parser, RFC 8785, envelope, digest framing, future reference shape, 21 domains, and 8 version axes are frozen for review. WorkIntent 0.3 binds exact raw source/planning candidates and the exact output schema while keeping target canonical digests null and forbidding raw-hash relabeling. Migration classes D1, D2, D3, and D4 are closed by measurement (273 profiled time paths, zero unprofiled; zero nonconformant fixture timestamps; zero generic decimals or binary numbers under the frozen typed scientific-decimal object); every migration class measures zero — the canonical profile is clear of migration blockers and remains unissued pending Gate A review and the operator's exact-byte decision; all 668 digest fields carry accepted scope annotations under the decided commitment construction; all eleven new profile/work-identity schemas add zero blockers in those classes |
...
absent. Four historical resource identities
...
| Work identity predecessor/successor cohort | Retains WorkIntent 0.1/0.2, canonical WorkLease 0.1/0.2, and WorkContract 0.2/0.3 while adding WorkIntentCore 0.2 and exact-reference WorkIntent 0.3 | Exact raw source/planning candidates and the output schema are bound, with raw lineage kept distinct from canonical identity. Profile issuance, target and WorkIntent canonical digests, admission, WorkLease/WorkContract rebinding, and the assignment cohort remain blocked. None is assignable or executable |
| Research event/trace 0.7.0/0.3.0 | Closed event branches and replay/adverse traces | Structural candidate; real EventContractRecords/reducers absent |
