...
| Canonical identity | 64 vectors and 4 metamorphic relations agree across pinned Python and Node implementations; the current audit covers 100 schemas and 198 fixtures | The profile is still blocked: 122 unprofiled time paths, 61 number/decimal findings, 668 unscoped digest fields, 56 divergent common-definition names, 11 unpinned profile bindings, and 238 nonconformant fixture timestamps |
...
| First-slice admission | Candidate 0.2 retains exact representational sets—43 required / 78 outside commands, 60 event types, 25 aggregate/state/reducer families, 11 owners, and one P0 prerequisite—while correctly reopening C5/PRQ-009 as blocking; 12 safe references and 21 known-bads pass the bounded adversarial checker | C5 lacks a constructible resolved WorkIntent and complete 13-event assignment binding. The remaining payload/member/state/reducer records, real P0/root/checkpoint/activation chain, composite replay/refinement proof, and review are also missing. Nothing is admitted or executable |

The current 100-resource schema set intentionally does not pretend to be a
complete offline resolver. Five referenced resource IDs have no matching local
current resource: missing `canonical-work-lease:0.1.0` (PRQ-008), historical
`command-contract-registry:0.1.0` and `command-receipt:0.3.0` consumers,
historical `work-contract:0.1.0` consumers, and the deliberately retained exact
