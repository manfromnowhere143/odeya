...
- **Evidence quality and guard coverage** —
  ...
  [0104](decisions/0104-require-stable-suite-reported-refusal-for-generalized-guard-proof.md)
  stable suite-reported refusal for generalized guard proof, and
  [0105](decisions/0105-govern-all-current-architecture-diagrams-as-rendered-release-evidence.md)
  the exact seven-diagram render contract.
...
- [`architecture/`](../architecture/) — retained machine records: guard- and
  ...
  Generated records are refreshed only by their declared authoring or audit
  tools; other records are deliberately authored and then machine-validated.
  No machine record gains scientific or Gate A authority from validation.
- [PRQ-002B product identity candidate suite](../tests/product-identity-profile-candidate/README.md)
...
- [PRQ-002E side-by-side profile construction](../tests/product-identity-profile-0.3-candidate/README.md)
  — twelve final-only successor schemas, nine structural-nonidentity fixtures,
  three ordered candidate records, and two source- and language-separated
  observers agreeing on the complete exact-byte fifteen-subject projection.
  It is construction evidence, not applicability-trace completeness,
  canonicalization conformance, identity, issuance, admission, or Gate A
  acceptance.
...
- [CANONICALIZATION_PROFILE.md](CANONICALIZATION_PROFILE.md) — the frozen
  predecessor identity/serialization candidate `odeya-jcs-0.1`; unissued. The
  separate scoped `odeya-jcs-0.2` successor remains unissued and is bound by
  ...
  retain one bounded two-member replay without amending the profile.
  [ADR 0103](decisions/0103-construct-side-by-side-odeya-jcs-0-3-candidate.md)
  retains the separate unissued `odeya-jcs-0.3`
  [core](../architecture/canonicalization-profile-core-0.3-candidate.json),
  [external evidence](../architecture/canonicalization-profile-0.3-candidate-evidence.json),
  and [migration candidate](../architecture/canonicalization-profile-0.2-to-0.3-migration-candidate.json).
  Those bytes are a construction candidate, not profile conformance or
  identity.
