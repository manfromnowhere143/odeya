...
The current proposed instance is [`architecture/module-dependency-manifest.json`](../architecture/module-dependency-manifest.json). Its offline [validator](../scripts/validate_module_manifest.py) presently proves the declared owner/module graph is acyclic, every declared dependency and layer edge resolves, all 30 logical modules obey the declared layer policy, all 47 machine aggregate types have one owner record, all 98 current schema files have one matching `$id`/owner record, and all 121 command plus 135 event discriminators derive one mapped aggregate owner. These are design-vocabulary, inventory, and owner/module-DAG checks, not an admitted command/event registry, a global-lock-order proof, or a reducer-identity proof. In particular, the validator does not resolve C7's `Verification` versus `Reproducibility` reducer identity/cardinality conflict and does not prove reducer correctness. The first-slice admission hypothesis is not yet dependency-exact: its execution/materialization, multi-grant, reducer-ownership, resource-algebra, and checkpoint/recovery prerequisites must close before its provisional counts can freeze, followed by real contract, state-subject, reducer, root, checkpoint, and activation records. This is architecture evidence only; it cannot prove future source imports or runtime isolation until actual packages exist.
