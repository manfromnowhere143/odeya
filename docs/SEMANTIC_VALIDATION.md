...
| EVT-001 | Logical `payload_type_id` exactly matches `event_type`; the event contract separately binds the exact enclosing `ResearchEvent` resource bytes and discriminator branch pointer. Under 0.7.0 the per-payload contract remains `unresolved_blocking`, its digest is null, and the event is not admitted, dispatchable, or replay-authoritative |
...
| AUTH-007 | The kernel selects exactly one contract-declared authority path from trusted context; a bounded N-slot first-slice path resolves N distinct action-instance/request/target grants in global role order and atomically emits exactly N matching use plus N exhaustion occurrences, while ingress/bootstrap/assignment-only, rejection, noop, and replay emit none |
...
| ADMISSION-006 | Envelope, receipt, and evidence bundle bind the same current RegistryActivation ID/sequence/digest, embedded P0 digest, root/C0, witnessed checkpoint, scope, and clear recovery-frontier digest; fork, quarantine, recovery ambiguity, epoch/root change, witness failure, expiry, or supersession rejects before mission facts |
...
| BUDGET-010 | One bounded VerificationRun owns one combined non-fungible operational/IV0-IV4 reservation; a second clean run owns a distinct reservation, and typed observation plus optional settlement share one atomic resource-observation batch only when the creation-bound money/non-money profile is complete |
...
| SCI-006 | `run.determine_validity` atomically originates RunValidity plus MeasurementDisposition; only valid/valid-measurement, valid/no-valid-measurement, and invalid/no-valid-measurement pairs are legal, and no current frontier exposes one half |
...
| VER-004 | `Verification` is the sole canonical reducer for lifecycle, reproducibility, replication, transport, dispute, invalidation, and independence axes; internal fold composition is deterministic and no event is reduced twice |
...
| CLAIM-004 | `claim.supersede` atomically commits supersession plus the monotone transitive invalid-source fence against exact ClaimVersion dependency references; split commits, stale frontiers, cycles, unavailable targets, or projection-only fanout cannot remain eligible/current |
...
| PROJ-004 | Receipt, checkpoint, reducer snapshot, and projection advertised as current end only at complete batch boundaries; an incomplete/reordered mandatory cohort quarantines the affected scope rather than being repaired by a reducer |
