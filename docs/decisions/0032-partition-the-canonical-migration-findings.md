...
- Gate effect: converts PRQ-001's raw migration findings (1,222 after ADR 0033,
  ADR 0034, and ADR 0035 corrected the audit's decimal detector three times;
  originally stated as 1,154) into nine
...
while the migration audit shows 1,222 findings across six classes, and every
...
shared `$defs`; 127 are one typed-object migration behind a per-field table; 11
...
proposal tables (127 decimal rows; 151 digest field groups), and
...
- propose the table contents. The 127 decimal rows need semantic type, unit, and
...
PRQ-001's decision surface drops from 1,222 findings to nine class decisions
