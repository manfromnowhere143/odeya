...
- Gate effect: converts PRQ-001's raw migration findings (1,179 after ADR 0033
  and ADR 0034 corrected the audit's decimal detector in both directions;
...
while the migration audit shows 1,179 findings across six classes, and every
...
shared `$defs`; 84 are one typed-object migration behind a per-field table; 11
...
proposal tables (84 decimal rows; 151 digest field groups), and
...
- propose the table contents. The 84 decimal rows need semantic type, unit, and
...
PRQ-001's decision surface drops from 1,179 findings to nine class decisions
