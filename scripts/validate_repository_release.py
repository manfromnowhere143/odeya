...
EXPECTED_INTEGRATED_VALIDATOR_SHA256 = (
    "130c3e6e7343b77bcf4eccdce3286b1b4d8a3b8995e3ded855b3cf497b28094c"
)
...
RELEASE_CONTRACT_KNOWN_BAD_MUTATION_COUNT = 9
...
EXPECTED_RELEASE_CENSUS_MUTATIONS = {
    ...
    "coherent-release-contract-growth-with-stale-prose": (
        ...
        "synthetic-tenth-release-contract-mutation",
        "release-contract expected and fixture censuses must each contain exactly 9 members",
    ),
}
EXPECTED_RELEASE_FIXTURE_README_MUTATIONS = {
    ...
    "stale-release-contract-fixture-readme-count": (
        ...
        "Eight release-contract mutations",
        "repository-release fixture README must carry exactly the "
        "executable-derived boundary 'Nine release-contract mutations'",
    ),
...
def validate_release_census_mutations(errors: list[str]) -> int:
    ...
    if not all(
        ...
        for value in (script_mutations, contract_mutations, census_mutations)
    ):
    ...
    for case in census_mutations:
        ...
        if not any(expected in issue for issue in mutation_issues):
            errors.append(
                f"repository release census mutation {case_id}: expected "
                f"{expected!r} refusal, got {mutation_issues}"
            )
            continue
        passed += 1
...
def validate_release_fixture_readme_mutations(errors: list[str]) -> int:
    ...
    if not all(
        isinstance(value, list)
        for value in (mutations, script_mutations, contract_mutations)
    ):
        errors.append(
