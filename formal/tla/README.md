...
Status: architecture evidence candidate, expanded 2026-07-16. These models check a finite abstraction of the proposed contracts. They are not runtime code, an implementation proof, a liveness proof, a physical safety case, or Gate A acceptance.
...
| `CompositeAuthorityResource.tla` | The resolved `verification.assign` → `attempt.start` authority/resource boundary with two exact five-role grant cohorts, one combined reservation, the start/invalidation/deadline race, and exact settlement evidence | Every accepted action consumes and exhausts all ordered action-bound slots atomically; rejection/replay consumes none; assignment creates one held reservation; start claims it; a preclaim terminal winner blocks start and a start winner blocks release/expiry; settlement requires every non-money actual plus billed/refunded money evidence, with missing/unavailable never coerced to zero |
...
The composite model closes the bounded interaction that the individual authority and reservation models intentionally did not combine. `verification.assign` and `attempt.start` each own a distinct ordered five-slot grant set (`safety`, `data_rights`, `resource`, `execution`, `verification`) bound to that exact action instance. Assignment atomically consumes/exhausts its set and creates one active combined hold. Start atomically consumes/exhausts its separate set and claims that hold. Preclaim invalidation and controlled-time expiry compete serializably with start: the first committed terminal outcome is monotonic. Exact non-money actuals and separate billed/refunded money axes are required for settlement; `missing`, `unavailable`, and `unknown` retain an out-of-domain sentinel. The four focused mutations admit a partial grant cohort, release after claim, inferred-zero partial settlement, or a stale start after controlled deadline expiry.
...
- the composite authority/resource graph uses one verification run, two action instances, five fixed ordered role slots per action, one combined unit ceiling over two abstract non-money axes plus money, one controlled-time deadline, and exact values in `0..1`; it models billed/refunded evidence and refund-not-above-billed but not currency, pricing, tax, exchange, metering precision, or multi-run capacity packing;
...
- transaction cohorts beyond the fixed assign/start pair, variable role profiles, partial multi-destination publication, nested delegation, multi-reservation arithmetic, lease renewal/revocation, checkpoints, recovery forks, rights graphs, and physical interlocks are not expanded here; and
